...
export async function setupDatabase() {
  ...
  const EXPECTED_VERSION = parseInt(process.env.EXPO_PUBLIC_API_DB_VERSION || "1", 10);
  ...
  try {
    ...
    if (fileInfo.exists) {
      ...
      try {
        ...
        const result = await databaseInstance.getFirstAsync("PRAGMA user_version");
        console.log("📊 Resultado PRAGMA:", result);
        
        const currentVersion = result?.user_version || 0;
        console.log(`🔍 Versión actual: ${currentVersion}, esperada: ${EXPECTED_VERSION}`);
